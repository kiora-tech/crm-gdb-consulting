...
class MassCustomerFixtures extends Fixture implements DependentFixtureInterface
{
    ...
    public function load(ObjectManager $manager): void
    {
        ...
        for ($i = 1; $i <= 100; ++$i) {
            ...
            $customer->setAddressPostalCode($postalCode);
            ...
            for ($j = 0; $j < $nbContacts; ++$j) {
                ...
                if ($this->faker->numberBetween(1, 100) <= 70) {
                    $contact->setMobilePhone($this->faker->phoneNumber());
                }
            ...
            for ($j = 0; $j < $nbEnergies; ++$j) {
                ...
                // Code unique
                $energy->setCode('CTR-'.str_pad((string) $i, 4, '0', STR_PAD_LEFT).'-'.($j + 1));

                // Fournisseur
                $energy->setEnergyProvider($this->faker->randomElement($energyProviders));

                // Type d'énergie (70% électricité, 30% gaz)
                $isElec = $this->faker->numberBetween(1, 100) <= 70;
                $energy->setType($isElec ? EnergyType::ELEC : EnergyType::GAZ);
                ...
                if ($isElec) {
                    // Puissance souscrite (en kVA) pour l'électricité
                    $powers = [36, 42, 48, 54, 60, 72, 84, 96, 108, 120, 132, 144, 156, 168, 180];
                    $energy->setPowerKva($this->faker->randomElement($powers));

                    // Consommations HP/HC
                    $totalConsumption = $this->faker->numberBetween(10000, 1000000);
                    $hpRatio = $this->faker->randomFloat(2, 0.6, 0.8);
                    $energy->setHpConsumption($totalConsumption * $hpRatio);
                    $energy->setHcConsumption($totalConsumption * (1 - $hpRatio));
                } else {
                    // Profil pour le gaz
                    $profiles = ['P011', 'P012', 'P013', 'P014'];
                    $energy->setProfile($this->faker->randomElement($profiles));

                    // Consommation totale pour le gaz
                    $energy->setTotalConsumption((float) $this->faker->numberBetween(5000, 500000));
                }
