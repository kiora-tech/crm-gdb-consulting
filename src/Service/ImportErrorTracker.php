...
{
    // Liste des erreurs d'importation, indexée par nom de fichier
    private array $importErrors = [];

    // En-têtes du fichier original
    private ?array $headers = null;

    // Métadonnées de l'import en cours
                ...
                foreach ($fileErrors as $error) {
                    ...
                    try {
                        ...
                        if ($this->headers) {
                            ...
                            if (is_array($rowData) && !isset($rowData['raw_data'])) {
                                // Données normales
                                $sheet->setCellValue('A' . $rowIndex, json_encode($rowData, JSON_UNESCAPED_UNICODE));
                            } else if (is_array($rowData) && isset($rowData['raw_data'])) {
                                // Données brutes de l'extraction
                                $sheet->setCellValue('A' . $rowIndex, json_encode($rowData['raw_data'], JSON_UNESCAPED_UNICODE));
                            } else {
